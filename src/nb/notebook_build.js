...
(function (NS) {
  ...
  function histogramPlotCell(histograms){
    ...
    code = code.concat([
      ...
      '            metrics = []',
      '            for pt in points:',
      '                categories.append(str(pt.get("category", pt.get("index"))))',
      '                count = pt.get("count")',
      '                if count is None:',
      '                    count = pt.get("value")',
      ...
      '                    metrics.append(f"{metric} {unit}".strip())',
      '                else:',
      '                    metrics.append(pt.get("rawLabel") or "")',
      '            x = list(range(len(counts)))',
      '            ax.bar(x, counts, label=serie.get("name") or None, alpha=0.7)',
      '            ax.set_xticks(x)',
      '            ax.set_xticklabels(categories, rotation=45, ha="right")',
      '            for xi, yi, label in zip(x, counts, metrics):',
      '                if label:',
      '                    ax.text(xi, yi, label, rotation=90, va="bottom", ha="center", fontsize=8)',
      '        ax.set_title(chart.get("title") or chart.get("kind") or "Histogram")',
      '        ax.legend(loc="best")',
      '    plt.tight_layout()',
      '    plt.show()',
